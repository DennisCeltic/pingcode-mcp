...
interface InsightWorkItem {
  ...
  assignee?: { id?: string; name?: string; display_name?: string };
...
function valueList<T>(data: unknown): T[] {
  if (Array.isArray(data)) return data as T[];
  const dict = data as { values?: T[] };
  return Array.isArray(dict?.values) ? dict.values : [];
}

function totalOf(data: unknown, fallback: number): number {
...
function daysSince(ts?: number, now = Date.now()): number | undefined {
  if (!ts) return undefined;
  return Math.floor((now - ts * 1000) / 86400000);
}

function nameOf(entity?: { name?: string; display_name?: string } | null): string {
  return entity?.display_name || entity?.name || '-';
}

function stateName(item: InsightWorkItem): string {
  return item.state?.name || '-';
}

function isDoneState(name?: string): boolean {
...
function isBlocked(item: InsightWorkItem): boolean {
...
function workItemLink(item: InsightWorkItem): string {
  const label = item.identifier ? `${item.identifier}: ${item.title}` : item.title;
  return item.html_url ? `[${label}](${item.html_url})` : label;
}

function buildWorkItemQuery(params: InsightScopeParams, pageIndex: number, pageSize: number): string {
  const query = new URLSearchParams();
  if (params.project_id) query.append('project_ids', params.project_id);
  if (params.sprint_id) query.append('sprint_ids', params.sprint_id);
...
function filterDone(items: InsightWorkItem[], includeDone?: boolean): InsightWorkItem[] {
  if (includeDone) return items;
  return items.filter((item) => !isDoneState(stateName(item)));
}

function sortBySeverity(items: InsightWorkItem[], now = Math.floor(Date.now() / 1000)): InsightWorkItem[] {
...
function tableRows(items: InsightWorkItem[], limit = 10): string {
  ...
  if (items.length > limit) out += `\n另有 ${items.length - limit} 个未展示。\n`;
  return out;
}

export async function generateProjectHealthReport(params: InsightScopeParams = {}): Promise<string> {
  ...
  if (overdue.length) actions.push(`先处理 ${overdue.length} 个逾期项，确认是否延期、拆分或关闭。`);
  if (unassigned.length) actions.push(`为 ${unassigned.length} 个无负责人工作项补齐 owner。`);
  if (stale.length) actions.push(`对 ${stale.length} 个停滞项发起同步，要求更新状态或阻塞原因。`);
  if (dueSoon.length) actions.push(`检查 ${dueSoon.length} 个临期项的验收标准和剩余工作量。`);
  if (actions.length === 0) actions.push('当前未发现明显风险，保持例会节奏和交付节奏。');
  out += actions.map((item, index) => `${index + 1}. ${item}`).join('\n');

  return out;
}

export async function summarizeWorkItemContext(params: WorkItemContextParams): Promise<string> {
  const item = await pingCodeClient.get<InsightWorkItem>(`/v1/project/work_items/${params.work_item_id}`);
  ...
  out += `## 最近评论\n\n`;
  try {
    const comments = valueList<Dict>(await listComments({ principal_type: 'work_item', principal_id: params.work_item_id }));
    if (comments.length === 0) {
      out += `无评论\n\n`;
    } else {
      for (const comment of comments.slice(0, commentLimit)) {
        const creator = comment.created_by as { display_name?: string; name?: string } | undefined;
        out += `- ${tsToDate(comment.created_at as number | undefined)} ${nameOf(creator)}: ${cleanText(String(comment.content || comment.body || ''), 180)}\n`;
      }
      out += `\n`;
    }
  } catch (error) {
  ...
  if (params.include_attachments ?? true) {
    out += `## 附件\n\n`;
    try {
      const attachments = valueList<Dict>(await listAttachments({
  ...
  const risks: string[] = [];
  const nowSec = Math.floor(Date.now() / 1000);
  if (!item.assignee?.id) risks.push('当前无负责人');
  if (item.end_at && item.end_at < nowSec && !isDoneState(stateName(item))) risks.push('已逾期且未完成');
  if ((daysSince(item.updated_at) ?? 0) >= 7 && !isDoneState(stateName(item))) risks.push('超过 7 天未更新');
  if (isBlocked(item)) risks.push('疑似阻塞');
...
export async function generateTeamLoadReport(params: TeamLoadParams = {}): Promise<string> {
  ...
  for (const item of items) {
    ...
    row.total++;
    if (item.end_at && item.end_at < nowSec) row.overdue++;
    if (item.end_at && item.end_at >= nowSec && item.end_at <= dueSoonSec) row.dueSoon++;
    if ((daysSince(item.updated_at) ?? 0) >= staleDays) row.stale++;
  ...
  if (startAt && endAt) {
    await Promise.all([...byAssignee.values()].map(async (row) => {
      ...
      try {
        const workloads = valueList<{ duration?: number }>(await listWorkloads({
          principal_type: 'work_item',
          report_by_id: row.id,
          start_at: startAt,
          end_at: endAt,
...
export async function scanDeliveryRisks(params: InsightScopeParams = {}): Promise<string> {
  const items = filterDone(await fetchWorkItems(params), params.include_done);
  const nowSec = Math.floor(Date.now() / 1000);
  const staleDays = params.stale_days ?? 7;
  const dueSoonSec = nowSec + (params.due_soon_days ?? 7) * 86400;

  const risks: Array<{ level: '高' | '中' | '低'; type: string; item: InsightWorkItem; reason: string }> = [];
  for (const item of items) {
  ...
  if (risks.length === 0) out += `| - | - | - | - | 未发现明显风险 |\n`;
  if (risks.length > 30) out += `\n另有 ${risks.length - 30} 条风险信号未展示。\n`;
