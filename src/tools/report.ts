...
function getWeekRange(): { startAt: number; endAt: number; monday: string; sunday: string } {
  const now = new Date();
...
export async function generateWeeklyReport(): Promise<string> {
  ...
  try {
    const myself = await pingCodeClient.get<{
      id: string;
      display_name: string;
      name: string;
      email: string;
    ...
    try {
      const team = await pingCodeClient.get<{ name: string }>('/v1/directory/team');
  ...
  let page = 0;
  while (true) {
    const data = await pingCodeClient.get<{
      values: Array<{
        id: string;
        identifier: string;
        title: string;
      ...
      total: number;
    }>(`/v1/project/work_items?updated_between=${week.startAt},${week.endAt}&page_index=${page}&page_size=100`);
  ...
  if (myId) {
    ...
    const wlValues = (workloadsForWIs as { values?: Array<{
      principal: { id: string; identifier: string; title: string; html_url: string };
      report_at: number;
      duration: number;
    }> }).values || [];
    ...
    for (const wl of wlValues) {
      ...
      if (!p?.id || workItemMap.has(p.id)) continue;

      try {
        const wi = await pingCodeClient.get<{ project: { name: string }; state: { name: string } }>(`/v1/project/work_items/${p.id}`);
  ...
  if (myId) {
    ...
    if (Array.isArray(workloadTypesData)) {
      for (const t of workloadTypesData as Array<{ id: string; name: string }>) {
        typeMap[t.id] = t.name;
      }
    }
    ...
    const workloadList = (workloads as { values?: Array<{
      id: string;
      duration: number;
      description: string;
      report_at: number;
    ...
    const total = (workloads as { total?: number }).total || workloadList.length;
    ...
    if (total === 0) {
      ...
      await Promise.all(workloadList.map(async (wl) => {
        if (wl.principal?.id && !projectMap[wl.principal.id]) {
          try {
            const wi = await pingCodeClient.get<{ project: { name: string } }>(`/v1/project/work_items/${wl.principal.id}`);
...
function extractDomain(identifier: string, projectName: string): string {
  const upper = identifier.toUpperCase();
  if (upper.startsWith('IOV')) return '🚗 IOV（车载）';
  if (upper.startsWith('IOT')) return '🏠 IOT（物联网）';
  if (upper.startsWith('RDZC')) return '🔬 RD（研究）';
  if (upper.startsWith('SC')) return '⚡ 深聪智能';
  if (upper.startsWith('CBZ')) return '🏢 CBZ（一鸣）';
  if (upper.startsWith('SW')) return '📋 事务管理';

  const projectUpper = projectName.toUpperCase();
  if (projectUpper.includes('IOV') || projectUpper.includes('日产') || projectUpper.includes('五菱') || projectUpper.includes('瑞驰') || projectUpper.includes('大众') || projectUpper.includes('大通')) return '🚗 IOV（车载）';
  if (projectUpper.includes('IOT') || projectUpper.includes('联想') || projectUpper.includes('矩阵麦') || projectUpper.includes('美的')) return '🏠 IOT（物联网）';
  if (projectUpper.includes('RD') || projectUpper.includes('研究')) return '🔬 RD（研究）';
  if (projectUpper.includes('深聪') || projectUpper.includes('SOLAR')) return '⚡ 深聪智能';
...
function buildSummary(items: SummaryWorkItem[], week: { monday: string; sunday: string }): string {
  const today = new Date();
  const todayStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;

  const isToday = (d: string) => d.startsWith(todayStr);

  const domainMap = new Map<string, { total: number; states: Map<string, number>; todayCount: number }>();
