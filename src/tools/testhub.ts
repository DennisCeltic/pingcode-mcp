...
export interface AddTestLibraryMembersParams {
  library_id: string;
  user_ids: string[];
  role_id?: string;
}

export async function addTestLibraryMembers(params: AddTestLibraryMembersParams) {
  ...
  for (const uid of params.user_ids) {
    ...
    const result = await pingCodeClient.post(
      `/v1/testhub/libraries/${params.library_id}/members`,
      body,
    );
    results.push(result);
  }
