...
interface WbsResult {
  stages: CreatedItem[];
  milestones: CreatedItem[];
  tasks: CreatedItem[];
  summary: string;
}

async function createWorkItem(body: Record<string, unknown>): Promise<{ id: string; identifier: string; title: string; html_url: string }> {
...
function buildSummary(result: WbsResult, errors: string[]): string {
  let md = '';
  md += `## WBS 分解创建完成\n\n`;
...
export async function createFromWbs(params: CreateFromWbsParams): Promise<string> {
  const errors: string[] = [];
  const stages: CreatedItem[] = [];
  const milestones: CreatedItem[] = [];
  const tasks: CreatedItem[] = [];
  ...
  for (const stage of params.stages) {
    ...
    try {
      ...
      if (stage.description) stageBody.description = stage.description;
      if (params.assignee_id) stageBody.assignee_id = params.assignee_id;
    ...
    for (const milestone of stage.milestones || []) {
      ...
      try {
        const msBody: Record<string, unknown> = {
          project_id: params.project_id,
        ...
        if (milestone.description) msBody.description = milestone.description;
        if (params.assignee_id) msBody.assignee_id = params.assignee_id;
  ...
  const result: WbsResult = { stages, milestones, tasks, summary: '' };
  result.summary = buildSummary(result, errors);
  return result.summary;
}

async function createTaskWithPhase(
  projectId: string,
  phaseId: string,
  task: WbsTask,
  tasks: CreatedItem[],
  ...
  try {
    ...
    if (task.start_at != null) body.start_at = task.start_at;
    if (task.end_at != null) body.end_at = task.end_at;
    if (assigneeId) body.assignee_id = assigneeId;
    if (task.assignee_id) body.assignee_id = task.assignee_id;
