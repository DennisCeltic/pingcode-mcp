...
import type { AuthState, AuthConfig, EnterpriseCredentials, PersonalCredentials } from './types.js';
...
async function setupEnterpriseBaseUrl(): Promise<{ baseUrl: string; isPrivate: boolean }> {
  const isPrivate = await askYesNo(
    '是否为私有部署的 PingCode？',
...
async function setupPersonalBaseUrl(): Promise<{ baseUrl: string; isPrivate: boolean; subdomain?: string }> {
  const isPrivate = await askYesNo(
    '是否为私有部署的 PingCode？',
    '私有部署需要输入您的 PingCode 服务地址，如 https://pingcode.yourcompany.com'
  );
...
async function setupTokenType(): Promise<'enterprise' | 'personal'> {
...
async function setupEnterpriseCredentials(): Promise<EnterpriseCredentials> {
  const clientId = await askInput('请输入 Client ID:');
  const clientSecret = await askPassword('请输入 Client Secret:');
  return { clientId, clientSecret };
}

async function setupPersonalCredentials(): Promise<PersonalCredentials> {
  const clientId = await askInput('请输入 Client ID:');
  const clientSecret = await askPassword('请输入 Client Secret:');
...
export async function runAuthSetup(): Promise<AuthState> {
  tokenManager.clear();
