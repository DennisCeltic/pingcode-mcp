...
function buildFriendlyError(issue, label) {
...
export function validateArgs(toolName, inputSchema, args) {
    ...
    if (result.success) {
        return { success: true, data: result.data, schema: zodSchema };
    }
    const errors = result.error.issues.map((issue) => buildFriendlyError(issue, toolName));
